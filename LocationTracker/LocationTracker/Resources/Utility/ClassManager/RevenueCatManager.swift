...
Purchases.logLevel = .error
    ...
    func fetchOfferings() {
        Purchases.shared.getOfferings { (offerings, error) in
            if let offering = offerings?.current {
                ...
                showToast(message: "Error fetching offerings: \(error?.localizedDescription ?? "Unknown error")")
            }
        }
    }
    ...
    func purchase(package: Package, completion: @escaping (Bool) -> Void) {
        Purchases.shared.purchase(package: package) { (transaction, customerInfo, error, userCancelled) in
            if let error = error {
                showToast(message: "Purchase failed: \(error.localizedDescription)")
                completion(false)
                return
            }
    ...
    func restorePurchases(completion: @escaping (Bool) -> Void) {
        Purchases.shared.restorePurchases { customerInfo, error in
            if let error = error {
                showToast(message: "Restore failed: \(error.localizedDescription)")
            }
            if let isActive = customerInfo?.entitlements.all[self.entitlementID]?.isActive {
                completion(isActive)
            } else {
